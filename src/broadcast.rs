...
pub async fn broadcast_presence<'a>(uuid: Uuid, unicast_ip: Option<String>)  {
    let make_msg = |ip:IpAddr,nt: &str, usn: &str| format!("\
        NOTIFY * HTTP/1.1\r\n\
    ...
    let msg_mediaserver = move |ip, uuid_urn: & str |make_dup(ip,uuid_urn,"urn:schemas-upnp-org:device:MediaServer:1");
    let msg_contentdir = move |ip, uuid_urn: & str |make_dup(ip,uuid_urn,"urn:schemas-upnp-org:service:ContentDirectory:1");
    let msg_connectionmanager = move |ip, uuid_urn: & str | make_dup(ip,uuid_urn,"urn:schemas-upnp-org:service:ConnectionManager:1");
    ...
    let broadcast =  || async move {
        ...
        for ip in ips {
            for _ in 0..3i32 {
                ...
                broadcast_message("mediaserver", &msg_mediaserver(ip,&uuid_urn), &unicast_ip).await;
                broadcast_message("connectionmanager", &msg_connectionmanager(ip,&uuid_urn), &unicast_ip).await;
